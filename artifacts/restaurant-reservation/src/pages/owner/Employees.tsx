...
const ROLES: EmployeeRole[] = ["Supervisor", "Senior Staff", "Host", "Waiter"];
...
function RoleBadge({ role }: { role: EmployeeRole }) {
  const colors: Record<EmployeeRole, string> = {
    ...
    "Senior Staff": "bg-blue-500/10 text-blue-400 border-blue-500/20",
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/5 bg-black/20">
                <th className="text-left px-6 py-4 text-xs uppercase tracking-wider text-muted-foreground font-medium">Employee</th>
                <th className="text-left px-6 py-4 text-xs uppercase tracking-wider text-muted-foreground font-medium">Username</th>
                <th className="text-left px-6 py-4 text-xs uppercase tracking-wider text-muted-foreground font-medium hidden md:table-cell">Phone</th>
                <th className="text-left px-6 py-4 text-xs uppercase tracking-wider text-muted-foreground font-medium">Role</th>
                <th className="text-left px-6 py-4 text-xs uppercase tracking-wider text-muted-foreground font-medium">Status</th>
                <th className="text-left px-6 py-4 text-xs uppercase tracking-wider text-muted-foreground font-medium hidden lg:table-cell">Date Added</th>
                <th className="text-right px-6 py-4 text-xs uppercase tracking-wider text-muted-foreground font-medium">Actions</th>
              </tr>
            </thead>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((emp) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-muted-foreground hidden md:table-cell">{emp.phone}</td>
                    <td className="px-6 py-4"><RoleBadge role={emp.role} /></td>
                    <td className="px-6 py-4"><StatusBadge status={emp.status} /></td>
                    <td className="px-6 py-4 text-muted-foreground hidden lg:table-cell">{emp.dateAdded}</td>
                    <td className="px-6 py-4">
                        ...
                        {confirmDelete === emp.id ? (
                          <div className="flex items-center gap-1 ml-1">
                            <button
                              ...
                              className="px-2 py-1 rounded text-xs bg-destructive text-destructive-foreground hover:bg-destructive/80 transition-colors"
                            >
                              Confirm
                            </button>
                            <button
                              onClick={() => setConfirmDelete(null)}
                              className="px-2 py-1 rounded text-xs border border-border hover:bg-white/5 transition-colors"
                            >
                              Cancel
                            </button>
                          </div>
            ...
            >
              <div className="bg-card border border-white/10 rounded-2xl w-full max-w-md shadow-2xl">
                ...
                <div className="p-6 space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="text-white">{selectedEmployee.dateAdded}</p>
                    </div>
                    <div>
                ...
                <div className="flex gap-3 p-6 border-t border-white/5">
                  ...
                  <Button className="flex-1" onClick={() => { closeModal(); openEdit(selectedEmployee); }}>
                    Edit Employee
                  </Button>
                </div>
              </div>

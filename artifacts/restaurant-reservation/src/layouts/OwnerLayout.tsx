...
export function OwnerLayout({ children }: { children: ReactNode }) {
  ...
  const navItems = [
    ...
    { label: "Reservations", path: "/owner/reservations", icon: Users },
    { label: "Calendar", path: "/owner/calendar", icon: CalendarIcon },
    { label: "Tables", path: "/owner/tables", icon: UtensilsCrossed },
  ...
  const SidebarContent = () => (
    <>
      <div className="p-8 pb-10 flex flex-col items-center gap-1 border-b border-sidebar-border">
        ...
        {owner && (
          <span className="text-xs text-sidebar-foreground/50 mt-1">{owner.username}</span>
        )}
      </div>
